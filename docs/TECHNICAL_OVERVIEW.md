...
- `rsync_to_laptop.sh`: pulls completed forward-index parquet parts into `$PM_EDGE_LOCAL_FORWARD_INDEX_DIR` and resolution outcomes into `$PM_EDGE_LOCAL_RESOLVED_DIR`. The resolved-output path defaults to a sibling `resolved_market_outcomes/` directory next to the forward-index archive. The script uses rsync size/mtime checks and `--partial`.
